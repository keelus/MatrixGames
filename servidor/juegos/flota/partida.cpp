...
bool flota::Partida::Iteracion(int socketId, MatrizLED *matrizLED) {
	...
	if (Turno == TiposTurno::TURNO_JUGADOR) {
		...
		TableroJugador.CoordenadaDeAtaque.X = 0;
		TableroJugador.CoordenadaDeAtaque.Y = 0;
		...
		while (!ataqueRealizado) {
			...
			contenidoPrincipal += "\n\nCoordenada de ataque actual -> (Fila: " + std::to_string(TableroJugador.CoordenadaDeAtaque.Y) + ", Columna: " + std::to_string(TableroJugador.CoordenadaDeAtaque.X) + ")";
			...
			case 'A':
			case 'a':
				if (TableroJugador.CoordenadaDeAtaque.X > 0)
					TableroJugador.CoordenadaDeAtaque.X--;
				break;
			case 'D':
			case 'd':
				if (TableroJugador.CoordenadaDeAtaque.X < 7)
					TableroJugador.CoordenadaDeAtaque.X++;
				break;
			case 'W':
			case 'w':
				if (TableroJugador.CoordenadaDeAtaque.Y > 0)
					TableroJugador.CoordenadaDeAtaque.Y--;
				break;
			case 'S':
			case 's':
				if (TableroJugador.CoordenadaDeAtaque.Y < 7)
					TableroJugador.CoordenadaDeAtaque.Y++;
				break;
			case ' ':
				if (TableroCPU.AtaqueYaRecibido(TableroJugador.CoordenadaDeAtaque)) {
					errorAtaqueYaRealizao = true;
					std::cout << "Ataque ya realizado!\n";
				} else {
					ataqueRealizado = true;
					Ataque resultadoAtaque = TableroCPU.RecibirAtaque(TableroJugador.CoordenadaDeAtaque);
